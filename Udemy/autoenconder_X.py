...
def momentum_updates(cost, params, mu, learning_rate):
    # momentum changes
    dparams = [T_shared_zeros_like64(p) for p in params]
...
class AutoEncoder(object):
    ...
    def createFromArrays(W, bh, bo, an_id):
        ...
        ae.W = theano.shared(W.astype(np.float64), 'W_%s' % ae.id)
        ae.bh = theano.shared(bh.astype(np.float64), 'bh_%s' % ae.id)
        ae.bo = theano.shared(bo.astype(np.float64), 'bo_%s' % ae.id)
...
def main():
    ...
    # vs
    dnn = DNN([1000, 750, 500])
    dnn.fit(Xtrain, Ytrain, Xtest, Ytest, pretrain=True, train_head_only=False, epochs=1)
